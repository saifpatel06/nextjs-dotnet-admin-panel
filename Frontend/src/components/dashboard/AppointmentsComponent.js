...
import { notify } from '../../../utils/notify'; // Direct import like Services
...
const AppointmentsComponent = ({ user, initialAppointments }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.time) return notify.error("Select a time slot!");

    notify.loading(isEditing ? "Updating..." : "Booking...");
    ...
    try {
        ...
        if (res.ok) { 
            setShowModal(false); 
            refreshAppointments(); 
            notify.success(isEditing ? "Updated!" : "Booked!");
        } else {
            notify.error("Could not save appointment");
        }
    } catch (error) {
        notify.dismiss();
    ...
    <div className={styles.content}>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 p-3 bg-white rounded shadow-sm border">
        ...
        <button 
          ...
          onClick={() => { 
            setIsEditing(false); 
            setFormData({id:null, clientId:'', barberId:'', serviceId:'', date:'', time:'', notes:'', status:'Pending'}); 
            setAvailableSlots([]);
            setShowModal(true); 
          }}
        >
            ...
            <div className="modal-content border-0 shadow-lg">
              ...
              <form onSubmit={handleSubmit}>
                      ...
                      {loadingSlots ? (
                        ...
                        availableSlots.map(s => (
                          <button 
                            key={s} 
                            type="button" 
                            className={`btn btn-sm ${formData.time === s ? 'btn-primary' : 'btn-outline-secondary'}`}
                        ...
                        <span className="text-muted small">No slots available for selection</span>
                      )}
                    </div>
                  </div>
                ...
                <div className="modal-footer bg-light border-0 d-flex justify-content-between">
                  {isEditing ? (
                    <button type="button" className="btn btn-outline-danger" onClick={() => handleDelete(formData.id)}>Delete</button>
                  ) : <div></div>}
